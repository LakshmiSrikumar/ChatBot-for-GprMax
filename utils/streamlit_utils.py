...
def chat_func():
    ...
    with st.sidebar:
        ...
        if st.session_state["model_type"] == "Base model":
            chat_model = st.selectbox("Select a model", ["deepseek-r1:1.5b","llama3.2"])
        else:
...
def eval_func():
    ...
    if st.button("Go!", key = "5"):
        if st.session_state['query_project'] == "":
            st.error("Please enter a project")
            st.stop()
        ...
        with st.spinner("Generating evaluation dataset"):
            ...
            if not st.session_state["eval_uploaded_files"]:
                st.error("No evaluation files uploaded")
                st.stop()
            ...
            if not qa_pairs or not isinstance(qa_pairs, list):
                st.error("Failed to generate valid QA pairs from uploaded files")
                st.stop()
            cleanup_uploaded_files(temp_eval_data_dir)
